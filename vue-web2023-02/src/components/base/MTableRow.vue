<template>
    <tr :class="['table--body',loadingTable ? 'skeleton' :'']" @dblclick="clickEditEmployee">
        ...
        <td :class="['table__MNV left',checkbox==true ? 'active' : '']" v-show="this.settingsDefault.employeeCode">
            <p @mouseenter="hoverDiv" class="tooltip-right">{{loadingTable==false ? data.employeeCode:''}}</p>
        </td>
        <td :class="['table__TNV left',checkbox==true ? 'active' : '']" v-show="this.settingsDefault.fullName">
            <p @mouseenter="hoverDiv" class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? data.fullName : ''}}</p>
        </td>
        <td :class="['table__GT left',checkbox==true ? 'active' : '']"  v-show="this.settingsDefault.gender">
            <p class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? formatGender : ''}}</p>
        </td>
        ...
        <td :class="['left',checkbox==true ? 'active' : '']"  v-show="this.settingsDefault.identityPlace">
            <p @mouseenter="hoverDiv" class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? data.identityPlace : ''}}</p>
        </td>
        <td :class="['table__CD left',checkbox==true ? 'active' : '']"  v-show="this.settingsDefault.position" >
            <p class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? data.positionName : ''}}</p>
        </td>
        <td :class="['table__TDV left',checkbox==true ? 'active' : '']"  v-show="this.settingsDefault.department" >
            <p class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? data.departmentName : ''}}</p>
        </td>
        <td :class="['left',checkbox==true ? 'active' : '']"  v-show="this.settingsDefault.phoneNumber">
            <p class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? data.phoneNumber : ''}}</p>
        </td>
        <td :class="['table__SALARY right',checkbox==true ? 'active' : '']"  v-show="this.settingsDefault.salary">
            <p class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? formatMoney : ''}}</p>
        </td>
        <td :class="['left',checkbox==true ? 'active' : '']"  v-show="this.settingsDefault.email">
            <p @mouseenter="hoverDiv" class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? data.email : ''}}</p>
        </td>
        <td :class="['left',checkbox==true ? 'active' : '']"  v-show="this.settingsDefault.address">
            <p @mouseenter="hoverDiv" class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? data.address : ''}}</p>
        </td>
        <td :class="['table__STK left',checkbox==true ? 'active' : '']"  v-show="this.settingsDefault.bankId">
            <p class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? data.bankId : ''}}</p>
        </td>
        <td :class="['table__TNH left',checkbox==true ? 'active' : '']"  v-show="this.settingsDefault.bankName">
            <p @mouseenter="hoverDiv" class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? data.bankName : ''}}</p>
        </td>
        <td :class="['table__CNTKNH left',checkbox==true ? 'active' : '']"  v-show="this.settingsDefault.bankAddress">
            <p @mouseenter="hoverDiv" class="tooltip-down1 tooltip-down-right">{{loadingTable==false ? data.bankAddress : ''}}</p>
        </td>
...
export default {
    ...
    methods:{
        ...
        /**
         * Khi nào số lượng phần tử vượt quá cho phép mới hiện ra tooltip
         * Author : Vũ Quốc Anh (24/03/2023)
        */
        hoverDiv(event){
            if(event.target.innerHTML.length >= 40){
                event.target.setAttribute('data-c-tooltip',event.target.innerHTML)
            }
        },
        /**
